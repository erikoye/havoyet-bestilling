...
def api_orders_new():
    ...
    if existing_idx is not None:
        ...
        if _is_pending_order_status(old_status) and not _is_pending_order_status(merged.get("status")):
            # Meta CAPI Purchase — kanonisk «betaling bekreftet»-punkt for ordre som
            # markeres betalt via re-POST (inkl. Vipps-app uten retur, dersom gateway/
            # klient re-poster hit). Deduppes m/ browser via event_id, idempotent.
            _fire_capi_purchase(merged)
            try:
